...
- Exercise search is handled by a shared liquid-glass picker in [`src/components/workout/overlays/SearchSheet.tsx`](/Users/sierra/Code/routyne/src/components/workout/overlays/SearchSheet.tsx). Desktop uses an inline two-column panel whose preview content scrolls independently so the commit button stays reachable. Mobile opens a bottom sheet with a compact selected-exercise bar, optional preview expansion, and a list-first result layout that keeps the results visible while you browse.
