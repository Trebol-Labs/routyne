...
- IndexedDB is the local source of truth; Supabase sync, push notifications, and the AI Coach are opt-in feature gates.
...
| B. Cloud Sync | Complete | Supabase auth, sync queue, merge engine, profile/history/bodyweight/routine sync, first-auth local bootstrap, history/bodyweight/routine tombstones | Push subscriptions are still in-memory on Hobby tier |
...
- Phase B is functionally complete for workout data. Push persistence is still a reliability gap if we want notifications to survive cold starts.
...
1. Verify the next `main` deploy on Vercel after the sync/auth changes land.
