...
export function RestTimer({ duration, onFinish, onClose }: RestTimerProps) {
  ...
  const timerNotificationsEnabled = useWorkoutStore((state) => state.profile.preferences.timerNotificationsEnabled);
  const { t, language } = useI18n();
  const localNotificationId = `routyne-rest-${notificationId.replace(/:/g, '')}`;
  ...
  }, [isRunning, targetTime, timerNotificationsEnabled, localNotificationId, t.notifications.restBody, t.notifications.restTitle]);
